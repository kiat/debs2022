...
The algorithmic trading of stock shares can be triggered by high complex event patterns that are specified based on 
patterns on the event stream of the stock market. Such algorithmic trading mostly are specified based on the 
history of the event stream, for example by utilizing moving averages of stock prices and the trading volumes. The real-time extraction of 
such complex patterns to trigger buy or sell trading actions is the task of high-performance event stream processing systems. 
...
The main challenge task was to process the stream of events and achieve high throughput and low latency. 
